...
// const Kraje = ["Cyprus", "Serbia", "Lithuania", "Ireland", "Ukraine", "Poland", "Croatia", "Iceland", "Slovenia", "Finland", "Moldova", "Azerbaijan", "Australia", "Portugal", "Luxembourg", "Germany", "United Kingdom", "Sweden"];
  ...
  return (
    <div>
      ...
      <div>
        ...
        <h2>Wybierz kraj odbierający punkty:</h2>
        <select id="krajOdbierajacy">
          {Kraje.filter(kraj => kraj !== krajPrzyznajacy).map(kraj => <option key={kraj}>{kraj}</option>)}
        </select>
        <button onClick={() => {
          const punkty = parseInt(document.getElementById('punkty').value);
          const krajOdbierajacy = document.getElementById('krajOdbierajacy').value;
